...
const GetVioDeptStudentList = async ({res, token, params}) => {
    ...
    try{
        if(sqlResult[0].is_token === 'AUTH') {
            dataResult.VioStudentList = await query(_sql.getViolationDeptStudentList, params)
        }
    } catch (err){
        error  = true;  
    }
    ...
    error ? 
        res.sendStatus(401) : 
        res.json({ data:dataResult.VioStudentList})
}

let _sql= {
    getViolationDeptStudentList : `SELECT  
                                    s.id as 's_id',
